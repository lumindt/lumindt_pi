...
class SQLUploader:
    ...
    # -------------------------------------------------------------------
    def upload_row(self, el_data, fc_data, alicat_data, test_id=None):
        ...
        try:
            ...
            values = {
                # Electrolyzer system
                "el_output_pressure": 0.0,  # Placeholder (not measured yet)
                "el_alicat_output": alicat_data.get("volumetric_flow_slpm", None),
                ...
                **{f"el_measured_production_rate_{i}": el_data.get(f"E{i}_prod_rate_NLh", None)
                   for i in range(1, 9)},
