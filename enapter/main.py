...
def run_enapters():
    ...
    while True:
        ...
        for idx, (ip, ctrl) in enumerate(controllers.items(), start=1):
            try:
                ...
                writer.writerow([
                    ...
                    f"E{idx}", ip,
                    "ON" if electrolyzer_on else "OFF", state,
                    f"{production_rate:.1f}", f"{electrolyte_temp:.1f}",
                    f"{stack_voltage:.1f}", f"{stack_current:.1f}",
                    f"{stack_power:.2f}", f"{stack_flow_rate:.1f}",
                    f"{h2_total_session:.1f}"
                ])
            except Exception as e:
                rows.append([f"E{idx}", ip, "ERROR", "-", "-", "-", "-", "-", "-", "-", str(e)])

        writer.writerow([curr_time.strftime("%Y-%m-%d %H:%M:%S"), "TOTAL", "-", "-", "-", "-", "-", "-", "-", f"{total_flow:.1f}", f"{total_h2:.1f} NL"])
        csvfile.flush()
        ...
        for i, (ip, ctrl) in enumerate(controllers.items(), start=1):
            try:
                errors = ctrl.display_error_codes()
                if isinstance(errors, list) and len(errors) > 0:
                    print(f"E{i} ({ip}) Error Codes: {errors}")
        ...
        for i, (ip, d) in enumerate(dryers.items(), start=1):
            try:
                errors = d.display_error_codes()
                if isinstance(errors, list) and len(errors) > 0:
                    print(f"Dryer {i} ({ip}) Error Codes: {errors}")
                elif isinstance(errors, str):
                    print(f"Dryer {i} ({ip}) Error Codes: {errors}")
                else:
